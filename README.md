...
**Make multi-AI handoffs get better on their own.** A write-time checker for handoff reports, plus a shared "lessons book" that learns from the mistakes AI reviewers catch in each other and rolls them out as one-line rules — with a 48-hour human veto window.
...
4. **Watchdog** (Windows). Pops a notification only when you are actually needed: a rule is about to take effect, a scheduled run failed, or one AI keeps failing checks. The popup just tells you what's going on; the decision page shows the full rule, why it exists, who proposed it, and two buttons — **Agree** / **Reject**.

## What you actually do

Two things, and neither is required:
...
python -X utf8 规范/交接写时门/tests/test_lessons.py   # 35 checks, fake model, no network
python -X utf8 规范/交接写时门/tests/test_notify.py    # 39 checks, no popups, no registry
```
...
Only the daily-learning step calls a model, at most once a day. The checker, watchdog, and scan call **no model at all**.
...
- **127 regression tests** that call the production code, run on Windows and Linux.
- Ran for about two days on one Windows 11 machine (publisher) and one Linux server (checks only).
- Claude Code hooks were verified to fire on both machines. **The ZCode hook was installed but never verified to fire.**
- Real daily-learning runs so far: 26 reports → 0 rules accepted (claude); 23 reports → 1 rule accepted (glm).
...
```
规范/交接写时门/     the scripts, their detailed docs, and the three test suites
